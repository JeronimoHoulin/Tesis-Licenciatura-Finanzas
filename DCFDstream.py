...
subyacente = brownian_motion(sub, 0.1, 0.025, 100, 0.1)
...
empty_sub = pd.DataFrame([sub-sub*0.05],columns=['Price'], index=[0])
...
def main():
    ...
    for i in range(1,len(subyacente)):
        ...
        empty_sub.loc[i+1] = subyacente[i]
        fig, ax = plt.subplots()
        ax.plot(empty_sub)
        fig.set_figheight(7)
        fig.set_figwidth(15)
        ...
        if user_long.active == True:  
            ...
            df.loc[i,'margin_long'] = df.iloc[-1,3]+df.iloc[-2,1] + long_add 
            df.loc[i,'margin_short'] = df.iloc[-1,4]+df.iloc[-2,2] + short_add
        ...
        order_book_s = order_book_s.sort_values(by=['Price']).reset_index(drop=True)
        metric_slot.metric(label='Underlying Price', value=round(subyacente[i],2), delta=round(subyacente[i]-subyacente[i-1],3))
