...
def create_orders(market_price,b, side):
    if side == 'buy':
        ...
        address = [''.join(rd.choice(adress_gen) for i in range(10)) for j in range(n)]
        return pd.DataFrame({'Price': buy_orders_price, 'Size':buy_orders_size, 'Address': address})
    ...
    if side == 'sell':
        ...
        address = [''.join(rd.choice(adress_gen) for i in range(10)) for j in range(n)]
        return pd.DataFrame({'Price': sell_orders_price, 'Size':sell_orders_size, 'Address': address})  


order_book_b = pd.DataFrame(create_orders(subyacente['price'][0], 2, 'buy'), columns=['Price', 'Size', 'Address']).sort_values(by=['Price'], ascending=False).reset_index(drop=True)
order_book_s = pd.DataFrame(create_orders(subyacente['price'][0], 2, 'sell'), columns=['Price', 'Size', 'Address']).sort_values(by=['Price']).reset_index(drop=True)
    ...
    try:
        while max(order_book_b['Price']) >= min(order_book_s['Price']):
            ...
            if order_book_b['Price'][0] >= order_book_s['Price'][0]:
                ...
                trades.loc[trade_n]=[price, size, i, contract_id]
...
def main():
    ...
    while True:
        ...
        if user.active == True:
            ...
            df.loc[i,'Price'] = trades.loc[trade_n ,'Price']
            ...
            print(df.risk_long, df.risk_short)
